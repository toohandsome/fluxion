...
4. 当 `technical-solution.md` 出于说明目的引用协议时，只允许保留“摘要 + 链接”，不得形成第二套可独立演化的字段定义。
5. 同一概念需要在多文档出现时，主文档写完整定义，其他文档仅写摘要并链接。
6. 若后续需要补充新的字段级协议，应优先新增或扩展对应 contract 文档，而不是继续堆叠到 `technical-solution.md`。
7. 若 `RAWDOC/*` 与正式基线冲突，以 `docs/base.md` 与 `docs/phase-1/*` 为准。
