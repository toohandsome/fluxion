...
```mermaid
graph TD
    A[流程设计人员 / 平台管理员] --> B[Fluxion Server]
    C[业务调用方] --> D[HTTP Endpoint / Scheduler Trigger]

    subgraph Server[Fluxion Server]
        B1[Admin API]
        B2[Runtime API]
        B3[Modeler]
        B4[Engine]
    ...
    subgraph Starter[Fluxion Starter]
        S1[Auto Configuration]
        S2[Embedded Engine]
        S3[Embedded Runtime API]
    end
...
- `Starter` 面向嵌入式接入，负责把引擎能力装配进业务系统，本身不作为独立服务启动
- `Server` 面向平台化使用，在 `Starter` 底座上额外提供独立 Web/API 能力、设计、发布、监控、调度和运行时触发
